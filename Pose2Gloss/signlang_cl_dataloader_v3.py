...
class AIHUB_SIGNLANG_DATASET(Dataset):
    def __init__(self, 
        ...
        if 'gloss' in modality: 
            ...
            if self.phase == 'train':
                def get_mode(x):
                    _,_,ws,_,_ = x.split('_')
                    ws = ws[:-4]
                    return ws 
                self.gloss_df['mode'] = self.gloss_df.file_id.apply(get_mode)
                self.gloss_df = self.gloss_df[self.gloss_df['mode']=='WORD']
            ...
            avail_id = avail_id<1.7
            avail_id = avail_id>0.8
            self.gloss_df2 = self.gloss_df[avail_id]

            avail_dic = self.gloss_df2.gloss.value_counts()>10
        ...
        # self.d = {}
        # for g, g_id in self.word2id.items():
        #     tmp_df = self.gloss_df[self.gloss_df.gloss==g]
        #     self.d[g]={'id':g_id,
        #                'n':len(tmp_df),
        #                'start':tmp_df.start.values,
        #                'end':tmp_df.end.values,
        #                'file_id':tmp_df.file_id.values,
        #                'duration':tmp_df.duration.values,
        #                'seq_len':tmp_df.seq_len.values,
        #                }
        
        self.d={}
        for i, g in enumerate(list(self.gloss_df.gloss.value_counts().index)):
            tmp_df = self.gloss_df[self.gloss_df.gloss==g]

            # tmp_pose = self.get_samples(gloss, start, end, file_id, duration, seq_len)
            self.d[g] = {}
            self.d[g]['n'] = len(tmp_df)
            self.d[g]['id'] = i
            self.d[g]['start'] = tmp_df.start.values
            self.d[g]['end'] = tmp_df.end.values
            self.d[g]['file_id'] = tmp_df.file_id.values
            self.d[g]['duration'] = tmp_df.duration.values
            self.d[g]['seq_len'] = tmp_df.seq_len.values
        self.dd = {v['id']:k for k,v in self.d.items()}

        self.keypoints = {}
        for file_id in self.gloss_df.file_id.unique():
            self.keypoints[file_id] = self.get_keypoints(file_id)

        print('end initialize')
    def __len__(self):
        if self.phase == 'train':
            return len(self.d)*100
        else:
    ...
    def __getitem__(self, idx):
        ...
        if self.phase != 'train':
            ...
            self.gloss2id = {g:i for i, g in enumerate(self.feature_db.gloss)}            
            ...
            gloss = self.dd[idx]
            v = self.d[gloss]
            sample_id = np.random.randint(0,v['n'],(self.num_pos_samples+1,))
            for sid in sample_id:
                file_id = v['file_id'][sid]
                start = v['start'][sid]
                end = v['end'][sid]
                keypoints = self.keypoints[file_id].copy()
                pose.append(self.get_pose(keypoints,start,end))
                # label.append(idx)

            anchor = np.tile(pose.pop(),(self.num_pos_samples,1,1,1))
            pos_sample = np.stack(pose)

            neg_gloss = list(self.d.keys())
            neg_gloss.remove(gloss)
            ng_id = np.random.randint(0,len(neg_gloss),(self.num_neg_samples,))
            pose = []
            for n_id in ng_id:
                gloss = neg_gloss[n_id]
                v = self.d[gloss]
                sid = np.random.randint(0,v['n'],(1,))[0]
                file_id = v['file_id'][sid]
                start = v['start'][sid]
                end = v['end'][sid]
                keypoints = self.keypoints[file_id].copy()
            ...
            neg_sample = np.stack(pose)

            return torch.tensor(anchor).to(torch.float32), torch.tensor(pos_sample).to(torch.float32), torch.tensor(neg_sample).to(torch.float32)
        negative_sample = []
    ...
    def get_samples(self, gloss, start, end, file_id, duration, seq_len):
        ...
        if self.use_pose:
            ...
            keypoints_seq = keypoints_seq[:,self.use_features,:]
            # Slicing
    ...
    def get_keypoints(self,file_id):
        _,_,gtid,ctid,view_point = file_id.split('_')
    ...
    def get_pose(self,keypoints_seq,start,end,):
        # Slicing
        seq_len = keypoints_seq.shape[0]
        if (end-start)>self.time_window: # time window 보다 길 경우
            can_start = max(0, start-self.time_window*0.1)
        ...
        for idx, p in enumerate(pose):
            ...
            img = np.ones((1080,1920,3))*255  # Convert to a numpy array if it's not already
            
            # Draw the pose on the image with numbers
            for i, point in enumerate(p):
                x, y = int(point[0]), int(point[1])
                if 0 <= x < 1620 and 0 <= y < 1080:  # Ensure the point is within image bounds
                    # Draw a green circle at the pose point
                    cv2.circle(img, (x, y), 10, (0, 255, 0), -1)
                    # Put the index number next to the pose point
                    cv2.putText(img, str(i), (x + 10, y + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0),2, cv2.LINE_AA)
